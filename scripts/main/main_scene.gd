...
func _setup_background_atmosphere() -> void:
	...
	# 하단 HUD를 위로 끌어올린다 (자원·다음 손패가 캐릭터 발치 근처에 오도록)
	var bottom := get_node_or_null("BattleField/VBoxLayout/BottomArea")
	if bottom is Control:
		(bottom as Control).offset_top = -(1080.0 - _HUD_TOP_Y)

	# 무대는 상단 _STAGE_H 까지만. 그 아래는 UI 전용 영역(어두운 판)으로 분리해
	# 캐릭터와 HUD가 겹치지 않게 한다.
	var bg_tex := load("res://assets/art/pixel-lab/battle_bg_stage.png")
	if bg_tex:
		var bg := TextureRect.new()
		bg.name = "BgPixel"
		bg.set_anchors_preset(Control.PRESET_TOP_WIDE)
		bg.offset_bottom = _STAGE_H
		...
		# UI 영역 배경 — 무대 아래를 덮는 어두운 판
		var ui_bg := ColorRect.new()
		ui_bg.name = "UiAreaBg"
		ui_bg.set_anchors_preset(Control.PRESET_BOTTOM_WIDE)
		ui_bg.offset_top = -(1080.0 - _STAGE_H)   # 카드 영역만 (210px)
		ui_bg.offset_bottom = 0
		ui_bg.color = Color(0.055, 0.06, 0.075, 1.0)
	...
	add_child(vig)
	move_child(vig, 3)   # UI 판 위, BattleField 아래
...
func _setup_pixel_stage() -> void:
	...
	var insert_at := 4   # Background·BgPixel·UiAreaBg·BgVignette 뒤, BattleField 앞
...
# === 캐릭터 애니메이션 (Mattz Art 스프라이트 시트) ===
# 프레임 96×84 가로 배열 시트. AtlasTexture로 프레임을 잘라 순환시킨다.
# 발끝이 프레임 y=62에 고정돼 있어(작가 작업) 시트가 바뀌어도 접지가 흔들리지 않는다.
const _KNIGHT_DIR := "res://assets/art/packs/knight/Sprites/with_outline/"
const _SHEET_FW := 96
const _SHEET_FH := 84
const _SHEET_FEET_Y := 62      # 프레임 내 발끝 y (실측)
# 애니메이션별 속도 — idle은 호흡이라 느리게, 공격·피격은 타격감 위해 빠르게
const _ANIM_FPS := 12.0          # 액션 기본
const _IDLE_FPS := 6.0           # idle 7프레임 → 약 1.2초에 한 번 호흡

func _fps_for(anim: String) -> float:
	return _IDLE_FPS if anim == "idle" else _ANIM_FPS

# 애니메이션 이름 → 시트 파일
const _ANIMS := {
	"idle": "IDLE.png", "attack": "ATTACK 1.png", "attack2": "ATTACK 2.png",
	"attack3": "ATTACK 3.png", "hurt": "HURT.png", "death": "DEATH.png",
	"defend": "DEFEND.png", "walk": "WALK.png", "run": "RUN.png",
}

var _anim_frames: Dictionary = {}     # 이름 → Array[AtlasTexture]
var _anim_node: TextureRect = null
var _anim_timer: Timer = null
var _anim_cur := ""
var _anim_idx := 0
var _anim_once := false               # 1회 재생(공격·피격) 후 idle 복귀


func _load_anim(anim: String, dir: String = _KNIGHT_DIR) -> Array:
	var key := dir + anim
	if _anim_frames.has(key):
		return _anim_frames[key]
	var sheet := load(dir + _ANIMS[anim]) as Texture2D
	if sheet == null:
		return []
	var out: Array = []
	for i in range(int(sheet.get_width() / float(_SHEET_FW))):
		var at := AtlasTexture.new()
		at.atlas = sheet
		at.region = Rect2(i * _SHEET_FW, 0, _SHEET_FW, _SHEET_FH)
		out.append(at)
	_anim_frames[key] = out
	return out


# 한 번만 재생하고 idle로 돌아가는 애니메이션 (공격·피격 등)
func play_anim_once(anim: String) -> void:
	if not _ANIMS.has(anim) or _anim_node == null:
		return
	_anim_cur = anim
	_anim_idx = 0
	_anim_once = true
...
func _anim_tick() -> void:
	if _anim_node == null or _anim_cur == "":
		return
	var frames: Array = _load_anim(_anim_cur)
	if frames.is_empty():
		return
	if _anim_idx >= frames.size():
		if _anim_once:                    # 1회 재생 끝 → idle 복귀
			_anim_cur = "idle"
			_anim_once = false
			_anim_idx = 0
			frames = _load_anim("idle")
		else:
			_anim_idx = 0
	_anim_node.texture = frames[_anim_idx]
	_anim_idx += 1
	if _anim_timer:
		var want := 1.0 / _fps_for(_anim_cur)
		if not is_equal_approx(_anim_timer.wait_time, want):
...
func _merge_player_cluster() -> void:
	...
	# HP를 머리 위로 빼냈으므로 좌하단은 자원+모듈만. 폭을 줄이고
	# 어두운 판을 깔아 무대에 떠 있지 않고 UI로 읽히게 한다.
	var ability := vbox.get_parent()
	if ability is Control:
		var ac := ability as Control
		ac.size_flags_stretch_ratio = 0.75
		if ac is PanelContainer:
			var box := StyleBoxFlat.new()
			box.bg_color = Color(0.045, 0.05, 0.065, 0.82)
			box.set_corner_radius_all(6)
			box.set_content_margin_all(12)
			(ac as PanelContainer).add_theme_stylebox_override("panel", box)
...
# 플레이어 HP를 캐릭터 머리 위로 (보스와 대칭 구성)
func _float_player_hp() -> void:
	if player_face_texture == null or hp_label == null:
		return
	var bar: Node = _player_hp_fill.get_parent() if _player_hp_fill else null
	var holder := VBoxContainer.new()
	holder.name = "PlayerHpFloat"
	holder.mouse_filter = Control.MOUSE_FILTER_IGNORE
	holder.add_theme_constant_override("separation", 3)
	add_child(holder)
	var bf := get_node_or_null("BattleField")
	if bf:
		move_child(holder, bf.get_index())

	hp_label.reparent(holder, false)
	if bar is Control:
	...
	var cx: float = player_face_texture.position.x + player_face_texture.size.x * 0.5
	var head_y: float = player_face_texture.position.y + _SHEET_HEAD_Y * _PIXEL_SCALE
	holder.position = Vector2(cx - w * 0.5, head_y - 46).round()
...
func _float_boss_hp() -> void:
	...
	var w := 240.0   # 우측 '다음 예고' 카드와 붙지 않도록
	holder.set_anchors_preset(Control.PRESET_TOP_LEFT)
	holder.custom_minimum_size = Vector2(w, 0)
	holder.size = Vector2(w, 52)
	# 스프라이트 중심이 아니라 '몸통' 중심에 맞춘다 — 버그베어는 철퇴가 왼쪽으로
	# 길게 뻗어 있어 bbox 중심이 실제 덩치보다 왼쪽으로 치우친다.
	var cx: float = boss_face_texture.position.x + boss_face_texture.size.x * 0.5
	# 프레임(96×84) 상단에는 빈 공간이 많다. 실제 머리는 프레임 y≈25부터이므로
	# 그 위에 띄워야 상단 페이즈 HUD와 겹치지 않는다.
	var head_y: float = boss_face_texture.position.y + _SHEET_HEAD_Y * _BOSS_SCALE
	holder.position = Vector2(cx - w * 0.5, head_y - 50).round()
...
func _setup_game_log() -> void:
	...
	if market_button and market_button.get_parent():
		...
		if tinfo is HBoxContainer:
			(tinfo as HBoxContainer).alignment = BoxContainer.ALIGNMENT_BEGIN
